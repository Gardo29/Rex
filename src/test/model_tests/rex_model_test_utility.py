...
class RexModelTestUtility(BaseDatasetsTest):
    ...
    def _rex_to_list(self, predictions):
        return sorted(
            [[user_id, item_id] for user_id in predictions for item_id in predictions[user_id]],
            key=lambda x: [x[self.USER_ID], x[self.ITEM_ID]])
    ...
    def _manage_predictions(self, predictions, key, sort_function, map_function):
        ...
        for user_id, group in groupby(sorted(predictions, key=key), key):
            ...
            filtered_prediction += mapped_group
        # sort result based on user id and then item id
        return sorted(filtered_prediction, key=lambda x: (x[self.USER_ID], x[self.ITEM_ID]))
