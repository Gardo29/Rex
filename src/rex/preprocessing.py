...
class PreprocessFunctionOnCopy(PreprocessFunction, ABC):
    def _transform(self, dataframe):
        return self._transform_copy(dataframe.copy(deep=True))

    @abstractmethod
    def _transform_copy(self, dataframe):
        pass
...
class LabelEncoder(PreprocessFunctionOnCopy):

    def __init__(self, feature, baseline=0):
        self.feature = feature
        self.baseline = baseline
        self._encoder = preprocessing.LabelEncoder()

    def _fit(self, dataframe: DataFrame | PreprocessedDataFrame) -> LabelEncoder:
        return self._encoder.fit(dataframe[self.feature])

    def _transform_copy(self, dataframe):
        dataframe[self.feature] = self._encoder.transform(dataframe[self.feature])
        return dataframe
...
class GroupByFunction(PreprocessFunctionOnCopy, ABC):
    ...
    def _transform_copy(self, dataframe: DataFrame):
...
class ToCOOMatrix(PreprocessFunction):
    ...
    def _transform(self, utility_matrix):
        user_encoder = preprocessing.LabelEncoder().fit(list(utility_matrix.index))
        item_encoder = preprocessing.LabelEncoder().fit(list(utility_matrix.columns))
    ...
    def _apply_recursive(self,
        ...
        if not functions:
            ...
            try:
                ...
                exception(f"'{function_name}' exception: {str(e)}")
                if self.force:
...
def auto_preprocess_weights_dataframe(dataframe: DataFrame,
    ...
    if tools.is_no_weights_dataframe(dataframe):
        if train_size:
            train, test = model_selection.generate_train_test(dataframe, train_size, random_state)
